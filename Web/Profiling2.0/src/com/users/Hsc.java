...
/**
 * Servlet implementation class Hsc
 */
@WebServlet("/hsc")
public class Hsc extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try(PrintWriter out = response.getWriter()) {
			...
			PreparedStatement ps = Database.getStatement("select * from  hsc where username = '"+username+"' and number = "+number+"");
			Map<String, String> mp = new TreeMap<String, String>();
			try {
				ResultSet rs = ps.executeQuery();
				if(rs.next()) {
					ps = Database.getStatement("update hsc set school = ?, board = ?, yog = ?, percentage = ?, stream = ? where username = ? and number = "+number+"");
					...
					ps = Database.getStatement("insert into hsc values(?, ?, ?, ?, ?, ?, ?)");
					ps.setString(1, school);
					ps.setString(2, board);
					ps.setInt(3, yoc);
