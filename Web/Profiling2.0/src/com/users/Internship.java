...
public class Internship extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try(PrintWriter out = response.getWriter()) {
			...
			PreparedStatement ps = Database.getStatement("select * from  internship where username = '"+username+"' and number = "+number+"");
			Map<String, String> mp = new TreeMap<String, String>();
			try {
				ResultSet rs = ps.executeQuery();
				if(rs.next()) {
					ps = Database.getStatement("update internship set profile = ?, organization = ?, location = ?, startdate = ?, enddate = ?, description = ? where username = ? and number = "+number+"");
					...
					ps = Database.getStatement("insert into internship values(?, ?, ?, ?, ?, ?, ?, ?)");
