...
/**
 * Servlet implementation class Pg
 */
@WebServlet("/pg")
public class Pg extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try(PrintWriter out = response.getWriter()) {
			...
			PreparedStatement ps = Database.getStatement("select * from  pg where username = '"+username+"' and number = "+number+"");
			Map<String, String> mp = new TreeMap<String, String>();
			try {
				ResultSet rs = ps.executeQuery();
				if(rs.next()) {
					ps = Database.getStatement("update pg set college = ?, startDate = ?, endDate = ?, stream = ?, cgpa = ?, degree = ? where username = ? and number = "+number+"");
					...
					ps = Database.getStatement("insert into pg values(?, ?, ?, ?, ?, ?, ?, ?)");
					ps.setString(1, college);
					ps.setString(2, startDate);
					ps.setString(3, endDate);
